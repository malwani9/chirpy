...
export async function handlerAllChirps(req: Request, res: Response) {
    let authorId  = "";
    const authIdQuery = req.query.authorId;

    if (typeof authIdQuery === 'string') {
        authorId = authIdQuery; 
    }
    ...
    const sortDirectionParameter = req.query.sort;
    if (sortDirectionParameter === "desc") {
        sortDirection = "desc"
    }
...
export async function handlerDeleteChirp(req: Request, res: Response) {
    ...
    if (!deleted) {
        throw new NotFoundError(`Failed to delete chirp with chirpId: ${chirpId}`);
    }
